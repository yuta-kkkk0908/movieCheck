...
1. タスク16（記録一覧ページング + API連動）
2. タスク17（総視聴数の内訳表示強化）
3. タスク18（統計からの管理導線）
4. タスク19（作品情報取得の一括更新）
5. タスク20（作品情報取得の安定化・可観測性）

## 追加バックログ（2026-02-27）

### 15. `DONE` Record list bulk actions (select all / selected delete)

- 背景:
  - 記録一覧は1件ずつ削除のみで運用コストが高い。
- 目的:
  - 一括選択・選択削除・全選択を提供し、記録整理の操作量を下げる。
- 実装仕様:
  - 記録一覧にチェックボックス列を追加（全選択/ページ内全選択/解除）。
  - 選択件数表示 + 選択削除ボタン（確認ダイアログ必須）。
  - APIは `DELETE /records/{id}` の逐次実行でも可。将来の一括API追加を見据えインターフェースを分離。
- DoD:
  - 50件以上で複数選択削除がUI上で完走する。
  - 削除後に選択状態が正しくリセットされる。

### 16. `TODO` Record list pagination (UI + server-side)

- 背景:
  - 現在は全件描画で、件数増加時に一覧操作が重くなる。
- 目的:
  - 大量データでも記録一覧を快適に操作できるようにする。
- 実装仕様:
  - フロント: ページネーションUI（page/pageSize）。
  - バックエンド: `GET /records/` の `skip/limit` を活用し、総件数を取得できる形に拡張（必要なら `total` 返却の新レスポンス形式へ変更）。
  - 将来の並び替えを見据えて `sort_by` / `order` の受け口を検討。
- DoD:
  - 200件超のデータで表示・ページ切替が実用速度。
  - ページ移動後も編集/削除/作品情報更新が破綻しない。

### 17. `TODO` Total views breakdown on dashboard

- 背景:
  - 「総視聴数」はあるが内訳が画面に出ていない。
- 目的:
  - 総視聴数の内訳（視聴方法・気分・評価帯・期間別）を可視化する。
- 実装仕様:
  - `statistics.overview` の既存キー（`viewing_method_stats`, `mood_stats`, `rating_distribution`, `recent_90_days`）をダッシュボード表示に反映。
  - 内訳ごとに件数・割合を併記。
- DoD:
  - ダッシュボードだけで「総視聴数の構成」が把握できる。
  - データ0件時の表示崩れがない。

### 18. `TODO` Management actions from statistics

- 背景:
  - 統計で異常値を見つけても、管理操作へ繋がる導線がない。
- 目的:
  - 統計→対象記録の確認・整理（削除等）を短導線化する。
- 実装仕様:
  - 内訳カードから記録一覧へフィルタ遷移（例: 視聴方法=other、評価なし）。
  - フィルタ状態でタスク15の選択削除を使えるよう連携。
- DoD:
  - 統計画面から2操作以内で対象レコードの削除実行に到達できる。

### 19. `TODO` Batch refresh for movie details

- 背景:
  - 作品情報取得が行単位で、欠損補完に時間がかかる。
- 目的:
  - 監督/公開日/公開年などの欠損を一括更新できるようにする。
- 実装仕様:
  - バックエンドに一括更新APIを追加（対象: 全件 or 欠損のみ）。
  - フロントに「一括で作品情報取得（欠損のみ/強制）」を追加。
  - 実行中の進捗表示とキャンセル不可メッセージを明示。
- DoD:
  - 100件規模で一括更新が完走し、失敗時も処理結果（成功/失敗件数）が返る。

### 20. `TODO` Detail refresh robustness and observability

- 背景:
  - 作品情報取得失敗時の原因がUIで追いにくい。
- 目的:
  - 取得失敗の原因可視化と再試行しやすい実装にする。
- 実装仕様:
  - 失敗理由（HTTP失敗/DOM欠損/タイムアウト）を分類して返却。
  - リトライ戦略（短い再試行 + 最終失敗）を整理。
  - 最終更新日時を映画単位で保持・表示。
- DoD:
  - 失敗時に「何が起きたか」がUIメッセージで判別可能。
  - 同一映画の過剰リトライを防止できる。
